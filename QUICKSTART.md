...
9. **Phase 4.5 派发**（三项分别按「**按模式 / 按条件 / 默认启用**」决定，**都不是「主人自由开关」**）：
   ...
   - **G14 中文 AI 痕迹闸**（**按条件自动启用**）：中文 + 学术/商业评论/行业分析 → **必跑**；轻量档（**2000-3000 字**，真源 = `字数判定表.md`）→ 走内置「G14 自检」；纯外语交付 → **不适用**；主人显式要求关闭 → 走「豁免 + 交付说明披露」窄口。8 类检测，0-2 类 Pass / 3-4 类 Warning（主控呈报 3 选 1）/ 5+ 类 Fail
