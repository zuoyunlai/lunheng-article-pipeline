...
# 2. 主控自动派 T1∥T2∥T6 三检索员并行检索
# 3. 主控自动派 T3 分析 → T4 写手 → T5 审计 → T7 终检
# 4. 交付一份带引用来源的高质量长文
```
...
2. **Phase 1 检索派发**：自动 spawn T1∥T2∥T6 三个独立检索员
3. **Phase 2 分析派发**：自动派 T3 分析员生成大纲
4. **Phase 2.5 主人确认**：主人过目大纲
6. **Phase 3 写作派发**：自动派 T4 写手
7. **Phase 3.5 主人洞察补充**（可选）
8. **Phase 4 审计派发**：自动派 T5 审计
9. **Phase 5 终检**：主控 T7 终检交付

**主人只在 4 个节点介入**：Phase 0 / Phase 2.5 / Phase 3.5 / Phase 5
...
| **T1 文献侦查** | 已发布学术文献检索 | Phase 1 |
| **T2 数据检索** | 已发布统计数据检索 | Phase 1（与 T1/T6 并行） |
| **T3 分析** | 论点拆解 + 大纲编写 | Phase 2（T1+T2+T6 完成后） |
| **T4 写手** | 正文撰写 | Phase 3 |
| **T5 审计** | 质量检查 + 修订任务书 | Phase 4 |
| **T6 案例检索** | 具体事件/企业案例检索 | Phase 1（与 T1/T2 并行） |
| **T8 批判伙伴** | 从反方攻击论证 | Phase 3.5（T4 v1 后） |

完整角色定义：详见 [`glossary.md § 一 核心角色`](glossary.md#一核心角色8-个)
...
完整定义：详见 [`glossary.md § 二 三层防御体系`](glossary.md#二三层防御体系)
...
类型决定后续如何处理（观点来源 vs 理论贡献，详见 glossary.md § 一 T4）。
...
**答：可以但严格限制。** 详见 [`glossary.md § 三 数据信任级别`](glossary.md#三数据信任级别v221-核心机制)：
- 🟢 已发布公开数据（最高信任）
- 🟡 主人投喂数据（中信任）
