...
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.25  # 建议 pin 具体版本
```
...
9. **Phase 4.5 派发**（三项分别按「**按模式 / 按条件 / 默认启用**」决定，**都不是「主人自由开关」**）：
   - **T9 同行评审**（**按模式**）：行业分析/学术论文默认开启，公众号默认关闭（主人可开启）；6 维度评分 → accept/minor/major/reject。T9 结果必须在对话中呈现给主人
   - **G14 中文 AI 痕迹闸**（**按条件自动启用**）：中文 + 学术/商业评论/行业分析 → **必跑**；轻量档（≤2000 字）→ 走内置「G14 自检」；纯外语交付 → **不适用**；主人显式要求关闭 → 走「豁免 + 交付说明披露」窄口。8 类检测，0-2 类 Pass / 3-4 类 Warning（主控呈报 3 选 1）/ 5+ 类 Fail
