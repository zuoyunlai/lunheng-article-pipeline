...
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.39  # 建议 pin 具体版本
```
...
> **宿主配置**：论衡是**纯 skill**（不建 agent、不依赖特定模型/渠道），**任意 OpenClaw 配置开箱可用（默认多 Agent 模式）**，token 统计走精确路径（子代理完成事件 `Stats:` 行 + 主控侧 session_status 工具），不需三级降级。详见 SKILL.md「执行能力边界」段。
...
#     T9 同行评审（按模式：行业分析/学术默认开、公众号默认关）+ G14 中文 AI 痕迹闸（Phase 0 显式勾选才跑：默认不启用、不因语言/体裁自动触发，8 类检测，Phase 3.6 与 T6 同批）+ 方法论足迹面板（默认启用）  <!-- G14 八类判定真源 = gates/14-中文AI痕迹-gate.md §二 + checkers/中文AI痕迹-checker.md（本节不重列八类） -->
# 4. 交付一份带引用来源的高质量长文
```
...
9. **Phase 4.5 派发**（三项分别按「**按模式 / Phase 0 显式勾选 / 默认启用**」决定）：
   - **T9 同行评审**（**按模式**）：行业分析/学术论文默认开启，公众号默认关闭（主人可开启）；6 维度评分 → accept/minor/major/reject。T9 结果必须在对话中呈现给主人
   - **G14 中文 AI 痕迹闸**（**Phase 0 显式勾选启用**，默认关闭、不因语言/体裁自动触发）：勾选后 → **必跑**；勾选且轻量档（**2000-3000 字**，真源 = `字数判定表.md`）→ 走内置「G14 自检」；未勾选 → 不跑；已启用后主人显式要求关闭 → 走「豁免 + 交付说明披露」窄口。8 类检测，0-2 类 Pass / 3-4 类 Warning（主控呈报 3 选 1）/ 5+ 类 Fail
