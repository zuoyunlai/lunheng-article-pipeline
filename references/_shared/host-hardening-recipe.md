...
>
> 🔻 **加固状态与默认口径**：不配置本配方 ⇒ 论衡**照常可运行**（启动不被拒 = 开箱可用），**架构不变**（仍是多 Agent 九角色），但子代理工具面可能超限 → Phase 0 须**显式披露「仅第①层软约束生效」**并**请主人裁决**（可要求主控接管相关 worker 节点），记 `status.md` → `加固状态: 未加固`。**「未加固即拒绝启动」仍否决**——与「开箱可用」定位冲突（v2.12.32 实测：严格口径曾致并行层自锁零产物）。**推荐的机械层 = 配方 0（`tools.subagents.tools.deny`）+ 配方 1（`maxSpawnDepth: 1`）**——让多 Agent 真正可启用。
