...
>
> 🔻 **未加固时的默认降级（v2.12.44 收紧为 fail-closed，回应外部审计）**：不配置本配方 ⇒ 论衡**照常可运行**，但**多 Agent 不再是默认**：Phase 0 须**显式披露「仅第①层软约束生效」**并**请求主人显式确认**；**未确认 ⇒ 单主控模式**（敏感题材无需确认即强制单主控），记 `status.md` → `加固状态: 未加固（已降级）`。**「未加固即拒绝启动」仍否决**——与「任意配置开箱可用」定位冲突（v2.12.32 实测：严格口径曾致并行层自锁零产物）。**推荐的机械层 = 配方 0（`tools.subagents.tools.deny`）+ 配方 1（`maxSpawnDepth: 1`）**。
