...
> **性质：可选、非前提。** 论衡仍是**纯 skill** —— **任意 OpenClaw 配置开箱可用**（启动不被拒），不要求、也不附带任何宿主配置，**不读取、不修改**宿主配置，也不对宿主的权限设定作任何前提假设。**『开箱可用』≠『默认多 Agent』**：默认 = 单主控（主控亲为、不 spawn）；多 Agent 须两条件齐 —— (1) 宿主按本文件配方 0/1 落地；(2) 主人在 Phase 0 显式确认。任一缺失 ⇒ 单主控。
...
>
> 🔻 **加固状态与默认口径**：不配置本配方 ⇒ 论衡**照常可运行**（启动不被拒 = 开箱可用），但**默认 = 单主控**（主控亲为、不 spawn）。Phase 0 须**显式披露「仅第①层软约束生效」**并**请求主人显式确认**多 Agent；**未确认 ⇒ 单主控模式**（敏感题材无需确认即强制单主控），记 `status.md` → `加固状态: 未加固（已降级）`。**「未加固即拒绝启动」仍否决**——与「开箱可用」定位冲突（v2.12.32 实测：严格口径曾致并行层自锁零产物）。**推荐的机械层 = 配方 0（`tools.subagents.tools.deny`）+ 配方 1（`maxSpawnDepth: 1`）**——让多 Agent 真正可启用。
