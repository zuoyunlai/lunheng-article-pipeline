...
>
> 🔻 **未加固时的默认降级（v2.12.39，回应外部审计）**：不配置本配方 ⇒ 论衡**照常运行**，但 Phase 0 须**显式披露「仅第①层软约束生效」**，且**敏感题材默认切单主控模式**，并记 `status.md` → `加固状态: 未加固（降级）`。**「未加固即拒绝启动」已否决**——与「任意配置开箱可用」定位冲突（v2.12.32 实测：严格口径曾致并行层自锁零产物）。
