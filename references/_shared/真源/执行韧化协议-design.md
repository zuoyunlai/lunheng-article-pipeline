...
**不做**任何主动探测性请求（不发送探测 ping、不预检模型）；LLM 可用性只在**首次实际调用**时通过响应时间与首 token 延迟观察判定。
...
> **背景**：v2.5.13 实战中 SVG→PNG 子代理派发时 DeepSeek-v4-flash 返回 `billing error`。当时主控曾以 exec 亲完成（主人现场知情的一次性授权；该工具现属硬禁用面，此为例外史实、不得据此复用）。v2.6.6 起该兜底固化为 **Phase 0 显式 opt-in 项**，默认关闭。
...
> **适用边界（v2.12.57 消歧义）**：本树**只用于「未启动 / spawn 丢失」**（tokens=0 ∧ history 停在初始 prompt）。**已启动但硬卡静默**（零产物 ∧ 超 `runTimeoutSeconds` ∧ 心跳未续）**不走本树** —— 按 [`../agents/00-主控-扩展职责.md`](../../agents/00-主控-扩展职责.md)「§不通过处理」状态机：主控亲写兜底 + status 记录 + 告知主人，**不重试**。
>
