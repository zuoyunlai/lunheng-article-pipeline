...
| **sha256 / bytes（M-Exist-2）** | **默认不验** | **主人侧量值**（主人填回）。未回填 ⇒ `pending_owner_verification`，不得判通过 |

### 3. 已知漏洞（实战暴露）

- **LLM 自评自过** = 同一模型既写又审，**有偏的 LLM 可能给自己放行**。缓解：v2.3.11 引入「主控强制 wait 30s + 复核」验证产物（#2 写盘后自检）。
...
- **M-Gate-Report v2.2.4 输出格式**（JSON schema）：[`references/_shared/真源/M-Gate-Algorithm-appendix.md §1`](M-Gate-Algorithm-appendix.md)
- **论衡哲学化**（4 版本合并）：[`references/_shared/真源/M-Gate-Algorithm-appendix.md §2`](M-Gate-Algorithm-appendix.md)
- **教训沉淀**：[`references/_shared/真源/M-Gate-Algorithm-appendix.md §3`](M-Gate-Algorithm-appendix.md)
