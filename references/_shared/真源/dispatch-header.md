...
> 🚫 **零 exec 哲学护栏（v2.12.43 精确口径）**：论衡 agent 的声明式边界由 [`permissions.md`](../../permissions.md) 的「禁用面（denied）唯一真源」块统一描述（本文件不重列，避免漂移）。这是本 skill 的调用纪律，不是要求主人修改宿主配置；OpenClaw 原生支持多 Agent，实际平台权限由 OpenClaw/宿主负责。📌 **口径两点**：① 「零 exec」只指执行类工具；② 主控另持编排与状态面，是多 Agent 流程的设计内能力，不授予子代理。
...
> - 若与本档白名单一致 ⇒ 在交接报告「做了什么」首句写 `能力自检：通过（<档位>）`，**并按「计数 + 高危类别具名」报告本会话工具面**（① 实际可见工具计数 ② 与本档声明的差值计数 ③ 高危类别具名——执行类 / 外发类 / 写入类 / 会话类；v2.12.74 收窄 P-3：实测超限可达 64 项，逐项枚举必退化成形式留痕；仍**禁止只写「通过」**；档位不符 ⇒ 当场回报主控、**不继续跑**，详见本文件 §收尾协议），随后正常开工。
...
> 1. **模型 fallback**（= 兑底 / 静默无响应切下一档）—— worker 角色内部「同 T 角色换档」（如 T7 主模型 0 tokens 时切下一档并在报告头部标注「T7 由 XX 兑底」），**主控不接管，T 角色不变**；真源 = `references/_shared/真源/模型候选池.md` §二
...
**收尾协议（v2.12.56 P-1；v2.12.74 R1 按 OpenClaw 2026.9.5 实测改写判据）**
写完交接报告后，**以正常最终消息结束回合** —— 该消息即交接摘要，会作为 completion event **尽力**送达主控（⚠️ v2.12.74 限定：平台对「唤醒请求方」有重试上限，实测 `requester settle wake deferred too many times` ⇒ **永久投递失败**——故主控**不得**以 completion event 作为唯一推进依据，推进判据以磁盘产物为准）。
- **禁止**调用 `sessions_yield` / `agents_wait` / `next_check` / `subagents` / `sessions_list` / `sessions_history`（v2.12.74 R1，锚定 OpenClaw 2026.9.5 实测）：裸 `sessions_yield` = 被平台**拒绝**（`No pending child completion is owned by this turn`）；其报错文本会**诱导** worker 补 `waitFor: "message"` ⇒ **挂起 run**（实测 14h+ 不结束、`result` 为空、需主控显式唤醒）——照抄报错文本即踩坑。两条路径（被拒 / 挂起）均禁用，worker 纪律不变。⚠️ 平台行为随版本漂移——换版本后须重跑探针（`scripts/runtime-capability-probe.py`）更新本判据（锚点真源 = `../../../reports/2026-09-20-capability-runtime-assertion-2026.9.5.md` §4）。
- 交接摘要**必须放进最终消息**，**不得**放进 `acknowledgment` 字段（该字段不从子代理回合发出，实际不送达）。
- **能力自检必须按「计数 + 高危类别具名」报告工具面**（可见计数 / 差值计数 / 高危类别具名），**禁止只写「通过」**；档位不符即当场回报主控，不继续跑。
- **Stats line 缺口处置（v2.13.2，E1-3 实测）**：子代理**不得估算、不得自报 token 数**（防编造）。若平台未在 completion event 末尾回传 Stats line，子代理在**最终消息末尾**写一行 `Stats: unavailable（平台未回传 Stats line）`；主控据此在 `status.md` §4.7 记 **`stats_missing`** 标志，并在交付说明「成本指标」段**显式列出缺口行数**，**禁止**用会话文本行 tokens 冒充权威值（两个 token 口径不同，见下）。**连续同型缺口须在反哺报告计数**——实测 2026-09-22《认证政治》与 2026-09-25《筛选竞赛的均衡》为**连续两轮同型**（主控侧精确、子代理侧全缺）。
