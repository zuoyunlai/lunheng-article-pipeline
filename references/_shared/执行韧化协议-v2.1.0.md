...
> **v2.2.11 范围调整**：原 § 4.5 中的 spawn 日志诊断方法（读取 `~/.openclaw/agents/<agent>/sessions/*.trajectory.jsonl`）原本在主流程中提及，但**该路径属于 OpenClaw runtime 内部诊断**（不在论衡主流程职责范围内）。
...
> grep -c '"sessions_spawn"' "$F"   # spawn 次数
> grep -c '"sessions_yield"' "$F"   # yield 次数
