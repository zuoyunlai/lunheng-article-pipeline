...
> 背景：v2.1.8 主控一次性 spawn T1 + T2 + T6 三个独立 sessions_spawn（同一 function_calls 块内），三方并行。与传统单 spawn 不同，主控需额外监控：

- **三方并行心跳看总和**：T1+T2+T6 三个 sessions 都心跳后才视为「三方并行存活」——任一 30s 未心跳 = 该 T 心跳异常
