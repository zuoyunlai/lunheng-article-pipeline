...
### 3.5 配额耗尽识别 + 手工兜底（v2.5.18 新增，教训 #184 实战）
...
**识别（5 秒内出判定）**：
- 子代理 5 分钟内连续返回 `billing error` / `quota exceeded` / `insufficient balance` / `429 Too Many Requests`（带「rate limit」）
- 任一关键词命中 → 视为「配额类失败」（**不是网络/超时类失败**——后者走 ping 超时 fallback）

**处理（v2.5.18 P0 硬步骤）**：
1. 立即在 status.md 写 `[配额耗尽 HH:MM] primary=billing_error, 提示主人决策`
2. **不再自动切 fallback**（避免把配额问题传染给下一档——配额耗尽是账户级问题，不切单模型能解）
3. 主控在对话里**明确告知主人**「本轮 LLM 配额可能耗尽，3 选项：① 主控用 exec 工具亲完成该子代理任务（主控有 exec，论衡没有）② 主人切 OpenAI/Google/anthropic provider 重试 ③ 等下个计费周期」
4. **主控默认走选项 ①**（不等主人拍板，避免流水线卡死）—— 论衡 zero exec 的边界适用于子代理，**主控不受此限**
5. 实战背景：崩老头 v2.3「DeepSeek 配额耗尽双失败 → 切 MiniMax-M3 重试成功」是手动切 fallback 成功案例，但 1 次就耗双模型，**不是可持续模式**；v2.5.13 实战「主控 exec 兜底」是更优解（主控有 exec）
