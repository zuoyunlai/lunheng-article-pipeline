...
- **🔔 心跳周期性写入**：运行期间按心跳协议（启动 + 每约 5 分钟）**仅写入自己的心跳文件** `run/<项目名>/.tmp/<角色>-heartbeat.md`（轻量进度行，供主控监控）。`status.md` **由主控独占写入**，子代理不直接写。**披露 ≠ 同意**：该周期性写入**不因「运行本 skill」而自动生效**，须由 Phase 0 显式同意关卡放行（真源 = [`关键协议.md`](关键协议.md) §确定性同意门）；**未取得同意前不写 `.tmp/`**（fail-closed）
