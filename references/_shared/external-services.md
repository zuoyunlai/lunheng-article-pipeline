...
> **本表是「外发服务类别」的唯一真源（共 4 类）**：SKILL.md / `permissions.md` / 任务简报模板 / `关键协议.md` 一律**引用本表**，**不得各自重列**——重列必然漂移（历史上曾出现 4 份互斥清单：本表类别数与 SKILL.md / Opt-in 行 / frontmatter / description 各自不同）。
>
