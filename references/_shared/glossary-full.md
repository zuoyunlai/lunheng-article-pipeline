...
6. **时间锚点显式化 + 文件保留原则**：所有卡片引用必带年份；案例卡额外填「检索截止日期」+「事件时间窗口」；任一 Phase 失败或项目收尾，**论衡 agent 不执行任何删除操作**——已写文件保留在 `run/<项目名>/` 供主人手工清理（防误删红线）；项目归档同样由主控出清单、主人手工 `mv`/`cp`（agent 不执行），详见 [`project-archive-sop.md`](references/_shared/project-archive-sop.md)。**「归档」= 复制/移动到 `run/.archive/`；「删除」= 主人自行处理**——两个动作都由主人在 host shell 完成，论衡 agent 一律不碰。
