...
7. **可选项准入判据（本修订定案）**：**只给「有真实成本或真实取舍」的东西开关**（外发数据 / 花钱的 API / 额外产物）；**零成本的质量门与透明度项由条件决定，不由偏好决定** —— 分三态：`默认启用`（无开关，如方法论足迹面板）/ `显式勾选`（有真实成本或取舍者，如 G14 闸、学术元数据 opt-in）/ `按模式`（如 T9 同行评审）。要关闭只能走「**显式豁免 + 交付说明披露**」窄口。配套铁律：**一条款一真源**（类别清单只在一处定义，其余文档只引用不重列；重列必漂移）、**工具级 opt-in 与服务级外发同意分列**（绝不合成一张表）、**流程真源必须能表达可选性**（`phase-order.yaml` 节点带 `condition` / `degrade` / `exempt`）。详见教训 #338。
