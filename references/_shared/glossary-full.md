...
> **v2.3.0 重构**：角色编号重构——T6 案例检索 → T3 案例检索（三方并行检索员连贯 T1∥T2∥T3），T3-T8 顺延，T7/T8 交换位置（终检 → T8 主控亲完成、批判 → T6 独立早期攻击、审计 → T7 形式审查）。**编号 = 流水线 Phase 顺序**：T1-T3 检索 / T4-T5 加工 / T6-T8 防御。教训 #275。
...
> **v2.4.0 更新**：原「5 层」→「4 层」——skill 化后论衡不再依赖 OpenClaw 配置（openclaw.json），原第 3 层取消。
