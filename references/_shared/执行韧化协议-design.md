...
- **每 5 分钟一次**：在**自己的心跳文件** `run/<项目>/.tmp/<角色>-heartbeat.md` 追加心跳时间戳（格式：`[心跳 HH:MM] model=<model-id>`）；`status.md` 由主控独占写（主控读心跳后转写）
...
每段 ack 在自己的心跳文件 `run/<项目>/.tmp/<角色>-heartbeat.md` 追加： `[ack N% HH:MM] <一句话说进度>`；`status.md` 由主控独占写（主控读心跳后转写）。
