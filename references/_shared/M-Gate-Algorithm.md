...
| `证伪依据` | **仅当 `原文值` 与 `裁定值` 不一致时必填**：逐条枚举命中的具体位置（文件 + 行号 + 原文片段）、真阳性模式扫描结果（该模式在一份**已知合规样本**上命中数应为 0）、规范冲突说明（哪条规则与哪条规则互斥） | 主控 `read` + 推理填写 |

**判定铁律**：

1. **两者一致时** `证伪依据` 写「一致，无需证伪」——**留空 = 报告不完整**。
2. **两者不一致时**，三项内容（命中位置枚举 / 真阳性模式扫描 / 规范冲突说明）**缺任一项即视为伪造报告**——按 **P0** 处理，T8 不得签字放行。
3. **不得用「汇总数」代替逐条枚举**：只写「命中 3 处」而不列这 3 处的文件 + 行号 = 未枚举。
4. **`原文值` 不得转述**：必须能回到磁盘原件逐字核对；改写、概括、四舍五入后的值不是原文值。
5. **判定权不下放**：三项字段由 LLM 推理填写，不新增脚本执行路径，**不得**因此要求 agent 跑任何命令。
...
| 分片 | 内容 | 加载 |
|---|---|---|
| **必读**（本文件）| **可复核判定协议（双字段 + 证伪证据）/ 判定结果分档（四档）/「报告后激活」重跑时序** + 执行模型 / M-Form 8 项 / M-Exist 3 项 / M-Integrity 2 项（规则 + 伪代码）| 🟠 跑 M 门前 + T8 终检前**必读** |
...
3. ✅ **依次执行 M-Form 8 项 + M-Exist 3 项 + M-Integrity 2 项**（按本规约伪代码推理，含升级算法）；每项同时填 **`原文值` / `裁定值` / `证伪依据`** 三字段
