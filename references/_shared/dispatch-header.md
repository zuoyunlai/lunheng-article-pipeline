...
> 🚫 **零 exec 哲学护栏**：论衡 agent 全部禁 `exec` / `process` / `code_execution` / `browser` / `apply_patch` / `message` / `gateway` 等 **40 项特权工具** —— **全表唯一真源 = `SKILL.md` frontmatter `metadata.tools.denied`**（本文件不重列；重列即漂移风险）。⚠️ **声明式**：须**宿主配置**才真正生效（`metadata.tools` 为自定义声明，加载器不执行；沙箱默认关）。
