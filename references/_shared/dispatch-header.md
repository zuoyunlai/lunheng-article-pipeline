...
> 🚫 **零 exec 哲学护栏（v2.12.43 精确口径）**：论衡 agent 全部禁 `exec` / `process` / `code_execution` / `browser` / `apply_patch` / `message` / `gateway` 等 **40 项特权工具** —— **全表唯一真源 = `SKILL.md` frontmatter `metadata.tools.denied`**（本文件不重列；重列即漂移风险）。⚠️ **声明式**：须**宿主配置**才真正生效（`metadata.tools` 为自定义声明，加载器不执行；沙箱默认关）。📌 **口径两点**：① 「零 exec」**只指执行类工具**，`denied` 里的 `browser`/`terminal`/`computer`/`nodes` 等**非执行类**工具由该清单另行覆盖；② **主控另持编排与状态面**（`coordinator_only` 8 项）= 派发 / 收报告 / 记账的**设计内必需**能力，**非**「零 exec」的例外，也**不授予子代理**。
...
> - **同意门（fail-closed，逐调用过）**：**服务级外发（4 类，唯一真源 = [`external-services.md`](external-services.md) 逐类表：检索类 / 学术元数据 opt-in / 封面图像 opt-in / 抓取）+ 工具级 opt-in（仅封面 `image_generate`）**，**调用前**必须先核对任务简报 §0 的结构化同意记录（两轴逐项取值）；「**记忆**」**不在本门**（v2.12.35 删记忆辅助，服务级外发 5→4 类；且 `denied` 已含 `memory_store` / `memory_forget`）；放行判据与阻断动作见 [`关键协议.md`](关键协议.md) §确定性同意门。主人选 ④ 时**一次都不调**；记录缺失/含糊/不一致 = 按拒绝处理，停止并回报主控。

> 🧪 **启动自检（第一步，先于任何读写）**：开工前先看**我实际拿到的工具面**，对照自己那一档的白名单。**铁律先记：工具面超限 ≠ 获得任何调用许可**——超限只说明宿主未加固，**不授权我调用**其中任何一个工具（v2.12.32 修订：**工具面 ≠ 调用**）。处置按四档：
...
> - **② 工具面超限 = 警告级（加固缺口登记，不中止）**：本档白名单之外的工具出现在工具面里——**在未加固宿主上这是常态而非异常**（v2.12.32 实测：宿主未剥除子代理编排类工具）。它**只**说明「宿主加固缺口」，须**记录 + 披露**，并**照样开工**；**不构成任何工具的调用授权**。⚠️ 按旧口径据此中止 ⇒ **整条流水线在第一阶段自锁、T2/T3 零产物**（v2.12.32 实测）。**根治手段是宿主加固**（配方 → [`host-hardening-recipe.md`](host-hardening-recipe.md)），不是让 worker 停工。
...
> **同意门（v2.12.44 收紧，fail-closed）**：心跳写盘的**唯一**依据 = 主人对 Phase 0「将创建的文件清单」的**显式确认**，且清单**逐项**含 `.tmp/<角色>-heartbeat.md`。**主控自动列清单 / 主人沉默 / 「本来就要写盘」的推断，都不构成同意**。缺该确认 ⇒ **不写 `.tmp/`、不开工**（fail-closed）。
