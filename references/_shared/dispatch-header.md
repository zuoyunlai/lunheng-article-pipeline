...
> **子代理工具白名单（5 档角色最小工具集声明，v2.7.12 修正）**：OpenClaw 2026.9.x 的 `sessions_spawn` **无 toolsAllow 参数**——子代理实际工具 = 主控有效工具策略快照 − 平台硬性剥除（gateway/agents_list/session_status/cron/message/sessions_send/conversations_*；叶子另剥 sessions 系列）。宿主必须用 config `tools.subagents.tools.allow/deny` 收紧（至少 deny exec/process/browser/apply_patch），否则无法保证零 exec。档位差异在工具层不可逐子表达时，以 prompt + 只读路径约束兜底。
