...
> - **② 工具面出现未声明工具 = 观测提示（不中止）**：当前会话实际可见工具多于本角色声明，并不代表宿主配置错误，也不要求主人处理。只记录“本角色未使用这些工具”的自检结果并照常开工；**出现于工具面不构成调用授权**。论衡不把它写成“未加固”状态，也不把它作为启动或交付失败。
...
> - 🧭 **自检对象 = 自身可见工具面，不是宿主配置**：本自检只基于当前会话自身可见工具面判断，不读取 `openclaw.json`、sandbox 或任何宿主配置文件；不要把自检结果翻译成“宿主已加固/未加固”。
