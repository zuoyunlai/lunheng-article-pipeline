...
- **教训 #77**：数据信任级别是论衡防御 prompt injection 的核心机制
- **教训 #79**：v2.2.0 M-Form-3 算法 bug → v2.2.1.2 升级 comm -13 diff
- **教训 #82**：v2.2.0 M-Exist-1 算法对「## 附」段误判 → v2.2.1.2 升级支持多种文末节
- **教训 #83**：v2.2.1 P0-C 在「表格格式数据卡」落地短板 → 数据卡-template.md 加表格 fallback 段
- **教训 #84**：v2.2.1 P0-C 必须支持「[Dxx] 编号 + 1.x 表格」双格式 → M-Form-6 + M-Exist-3 双格式支持
