...
> **何时用**：与 T6 对同一个 `current_draft` 同批调用；**Phase 0 显式勾选「启用 G14」**（默认关闭，不因语言/体裁自动触发）→ `enabled` 必跑；已启用且轻量档 → `selfcheck` 走内置自检；未勾选 → `not_enabled` 不跑；已启用后主人**显式要求关闭**（`exempted_by_owner`）→ 全项目不 spawn，且须在交付说明披露。
...
【交接报告】按 [`templates/交接报告-template.md`](../templates/交接报告-template.md) **七段**交付：做了什么（8 类判定 + verdict）/ 产物在哪 / 怎么验证 / 已知问题 / 下一步（Pass 继续 / Warning 呈报 3 选 1（不自动修订）/ Fail 强制修订）/ 状态机更新（主控代写）/ **AI 使用披露**；**token 消耗由主控记录，不属子代理回报段**。
```
