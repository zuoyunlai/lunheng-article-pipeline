...
> 🔻 **未加固运行时的默认口径（v2.12.44 收紧为 fail-closed，回应外部审计「声明的最小权限未被默认强制」）**：宿主未配置第②/③层时，论衡**不拒绝启动**（保持「任意配置开箱可用」承诺），但**多 Agent 不再是默认**：① Phase 0 告知主人「本次运行仅第①层软约束生效」并**请求显式确认**；② **未取得显式确认 ⇒ 直接单主控模式**（不 spawn、跳并行出网、检索工具逐项同意）——**敏感题材**（未公开研究 / 客户数据 / 受监管材料）**无需确认即强制单主控**；③ `status.md` 记 `加固状态: 未加固（已降级）`，并进交付说明「遗留风险」。**判据分层**：宿主是否已配 `tools.subagents.tools.deny` / `maxSpawnDepth` 由**主人核验并书面确认**；子代理侧**唯一可观测判据 = 启动自检回执**。**「未加固即拒绝启动」（A 档）仍否决**——与「任意配置开箱可用」定位冲突，且 v2.12.32 实测曾致并行层自锁零产物；**B 档本义即「默认降级单主控」，本次是把实现对齐该本义**。
>
> **⚠️ 为何不采纳（v2.12.41 补，回应外部扫描「加固即拒跑」建议）**：A 档（宿主未加固即**拒绝启动**）被否决的三条理由 —— ① **与技能定位冲突**：论衡是纯 skill（说明书），不附带也不要求宿主配置项，「开箱可用」是承诺；② **实测反例**：v2.12.32 严格口径曾致**并行层自锁零产物**（拒跑比降级更伤）；③ **替代方案已覆盖风险**：披露 + 默认降级单主控 + `status.md` 记 `加固状态` + 交付说明列遗留风险 —— **观测性与可控性都保留**，只把「强制」交给宿主。**v2.12.44 补（回应外部扫描「least-privilege 未强制 / 未 fail closed」）**：已按建议把**默认路径改为 fail-closed** —— 未确认加固 ⇒ 单主控（见上条 ②）；同时**推荐宿主落地** `tools.subagents.tools.deny` + `maxSpawnDepth`（配方 0/1）。整理后同时保留「**任意配置开箱可用**」与「**未加固不默认跑多 Agent**」两条性质。需完全机械化者按 `host-hardening-recipe.md` 配置宿主即可。
