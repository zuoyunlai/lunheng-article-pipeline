...
> 🔒 **加固判据（v2.12.43）**：`加固状态` 的**唯一判据 = 子代理启动自检回执**（子代理自报工具面），**不是**主控推断，也**不是**论衡读宿主配置（论衡永不读宿主配置）。未加固 ⇒ 子代理工具面可能超限（警告级：记录 + 披露 + 不授权调用），记 `加固状态: 未加固`。
...
> 🔻 **加固状态判据与默认口径**：① 宿主未配置第②/③层时，论衡**不拒绝启动**（启动不被拒 = 「开箱可用」），但子代理工具面可能超限（警告级，非阻断）。② 加固配方 0/1 的作用 = 把 5 档白名单从「声明」升级为平台强制，**不决定架构**（架构恒为多 Agent 九角色）。③ **敏感题材**可由主人要求主控接管涉密 worker 节点。④ `status.md` 记 `加固状态: 未加固` 并进交付说明「遗留风险」。⑤ 判据分层：宿主侧由主人核验并书面确认；子代理侧**唯一可观测判据 = 启动自检回执**。
>
> **⚠️ 为何不采纳 A 档（宿主未加固即拒跑）**：三条理由 —— ① 与纯 skill 定位冲突：「开箱可用」= 启动不被拒，不附带也不要求宿主配置项。② 实测反例：v2.12.32 严格口径曾致**并行层自锁零产物**（拒跑比降级更伤）。③ 替代方案已覆盖风险：本文件默认口径已含披露 + **节点级接管** + `status.md` 记 `加固状态` + 交付说明列遗留风险 —— 观测性与可控性都保留，「强制」留给宿主。**当前默认**：未加固 ⇒ 记录 + 披露 + 不授权调用超限工具，同时**推荐宿主落地** `tools.subagents.tools.deny` + `maxSpawnDepth`（配方 0/1）。需完全机械化者按 `host-hardening-recipe.md` 配置宿主即可。
