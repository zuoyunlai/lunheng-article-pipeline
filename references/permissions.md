...
> 🔒 **加固判据（v2.12.43）**：`加固状态` 的**唯一判据 = 子代理启动自检回执**（子代理自报工具面），**不是**主控推断，也**不是**论衡读宿主配置（论衡永不读宿主配置）。未加固 ⇒ Phase 0 披露 + 记 `未加固（降级）` + 敏感题材**强制单主控**。
...
>
> **⚠️ 为何不采纳（v2.12.41 补，回应外部扫描「加固即拒跑」建议）**：A 档（宿主未加固即**拒绝启动**）被否决的三条理由 —— ① **与技能定位冲突**：论衡是纯 skill（说明书），不附带也不要求宿主配置项，「开箱可用」是承诺；② **实测反例**：v2.12.32 严格口径曾致**并行层自锁零产物**（拒跑比降级更伤）；③ **替代方案已覆盖风险**：披露 + 默认降级单主控 + `status.md` 记 `加固状态` + 交付说明列遗留风险 —— **观测性与可控性都保留**，只把「强制」交给宿主。**外部扫描若仍报「least-privilege 未强制」：属设计取舍，非缺陷**；需机械强制的用户按 `host-hardening-recipe.md` 配置宿主即可。
