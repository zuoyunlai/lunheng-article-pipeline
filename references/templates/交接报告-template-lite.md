...
> 🏁 **收尾硬规则（v2.12.56 P-4，硬约束；与 P-1 同源）**：本交接报告的**正文/摘要必须随最终消息一起结束回合**（该消息即 completion event 送达主控）；**不得**塞进 `acknowledgment` 字段（该字段不从子代理回合发出，实际不送达——实测导致主控「完成事件 + 摘要」双丢）。**禁用** `sessions_yield` / `agents_wait` / `next_check` / `subagents` / `sessions_list` / `sessions_history`。详见 [`../_shared/dispatch-header.md`](../_shared/dispatch-header.md) §收尾协议。
