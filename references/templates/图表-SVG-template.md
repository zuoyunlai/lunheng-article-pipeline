...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 700 500" font-family="-apple-system, 'PingFang SC', 'Noto Serif CJK SC', 'Source Han Serif SC', serif">
  ...
  <!-- v2.12.42 对外层规定：上行内部编号仅用于审计层；交付/投稿版必须替换为规范著录（作者+年份 / 机构+年份），
       ...
       判据真源 = references/_shared/真源/可发表性判定表.md § 维度 5.4 -->
...
> **v2.12.52 定案**：封面视觉（`image_generate`）**不在流水线内**——论衡 agent 不调用该工具。本节仅保留数据图表口径；如主人需要封面，由主人在 T8 终检后按「主人自行操作建议清单」自行生成。

| 维度 | 数据图表（本模板） |
|------|-------------------|
| 生成方式 | 主控 `write` 手写 SVG |
| 数字可控 | ✅ 来自数据卡，100% 精确 |
| 是否需主人同意 | ❌ 否（SVG 本地生成零外发） |
| fallback | 无（SVG 永远可用，纯本地文本） |
| 图件位置 | `final/图件/图N_标题.svg` |
...
| 转换方式 | 执行性质 | 触发条件 | 谁来执行 |
|---------|---------|---------|---------|
| **SVG → PNG（公众号排版）** | 外部可执行程序（仅限人工操作） | T8 终检后建议清单 | **主人本人**本地手工跑命令；主控不执行任何转换 binary |
