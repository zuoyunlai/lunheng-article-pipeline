...
**运行性质**: 生产 / **测试模式**（测试模式 = phase2_5_outline / phase3_5_insight / phase5_acceptance 三个人在环节点自动通过；**必须在此 + 任务简报 + 交付说明三处同步披露**，v2.12.38）
**M 门**: v2.2.12 / v2.5.x
...
> 自检口径见 [`permissions.md`](../permissions.md)「能力自检」与 [`dispatch-header.md`](../_shared/dispatch-header.md)「启动自检」（**两级判据：工具面超限 = 警告，实际调用 = 阻断**）；**本表不记录宿主配置明细**（论衡不读宿主配置）。
...
> **留档（快照副本，非移动、非删除）**：项目结束时由主控**新增**一份本节内容副本到 `run/<项目名>/audits/methodology-footprint-{项目名}.md`——只写新文件，不移动、不删除任何既有文件（与 §5.8「结题归档」是两回事，后者由主人在工作流外手动完成）
...
**降级提示**：宿主配置清单中**无顶配档（批判审计）模型** → 主控**必须显式告知主人**「本机无顶配审计模型，审计/批判深度将降级，是否继续」——禁止静默降级。
...
- **留档**：项目结束时由主控 T8 终检**新增写入**一份方法论足迹副本到 `run/<项目名>/audits/methodology-footprint-{项目名}.md`（仅新增文件，不移动、不删除；与 §5.8 工作流外的「结题归档」区分）
- **借鉴 deep-research-pro 的方法论透明**（论衡化）——论衡不是简单复制竞品，而是把方法论足迹当成论衡哲学的一部分：诚实透明 + 主人随时看清进度 + 借鉴但不依赖
- **无需开关（本修订起定案）**：方法论足迹面板**默认启用**，不设自由开关；噪音控制走「**按档位裁剪字段集**」（轻量档保留最小字段集），而非整体关闭
