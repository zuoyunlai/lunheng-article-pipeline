...
>
> 🔒 **遥测分级（v2.12.39）**：本节「真实可用性 / 宿主可见余额」与全文 `session id` / `sessionKey` **仅留在本文件（status.md）**，**禁止转录进 `final/交付说明.md`**（交付说明只留 token 数 / 时长 / 模型组合）。
>
> ⚠️ **遥测收容四条（v2.12.40，回应外部安全扫描「遥测收容不足」）**：上句只管「不转录」，**未管 status.md 自身的暴露面**。本节另受四条约束（真源 = `_shared/关键协议.md` §遥测收容）：① 仅留本文件；② **归档/打包默认排除本文件**（留档只留脱敏副本 `status.redacted.md`）；③ **对外分享前必须脱敏**（`sessionKey`/`session id` 截前 8 + `…`；余额/可用性只留 ✅/⚠️/❌，不留数值）；④ 原文件随 `run/<项目名>/` 本地保留、项目清理即删。

| 能力档 | 角色 | 候选池（描述性，见模型候选池.md） | **本轮实际** | 真实可用性 | 宿主可见余额 |
|--------|------|-------------------------------|-------------|-----------|------|
