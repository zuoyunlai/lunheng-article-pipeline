...
## ▲ 降级运行记录（v2.7.3 新增 + v2.7.15 符号替换，fallback 每次一行，主人扫一眼可见）
...
## 三.五、M 门执行记录（v2.11.0 新增，教训 #235，主控每 phase 跑完登记）
...
| v1→v2 修订后 | 章节级 M 门（v2.10.0 新增）| YYYY-MM-DD HH:MM | 变更点 N 处 | ✅/❌ |
...
- ▲ **依赖外部**：论衡核心是 LLM 推理 + 文件读写 + Web 检索，若工具不可用自动降级（v2.3.13 模型收敛）
...
**真实可用性（v2.11.0 新增，教训 #236）**：Phase 0 模型自检升级为「**静态存在性 + 动态可用性**」双检查——静态读白名单确认模型 ID 存在，动态对每个候选模型做 1-token ping 记录真实返回码。「**配置存在 ≠ 可用**」：v2.10.3 首测顶配档 kkaiapi/claude-opus-5 配置在列但实际 403 认证失败，4 次 spawn 撞墙才暴露。顶配档全不可用 → 主控**显式告知主人**禁止静默降级。
