...
> ⚠️ **本文件含运行期遥测，按敏感文件处理（v2.12.40）**：`session id` / `sessionKey` / 宿主可见余额 / 模型可用性探测结果 / 阶段计时 / token 成本明细均属**运行期遥测**。**收容四条**：① **仅留本文件**（禁止转录进 `final/交付说明.md` 或任何交付物）；② **归档排除**——项目归档/打包默认**排除本文件**，确需留档时只归档**脱敏副本** `status.redacted.md`（见 [`../_shared/project-archive-sop.md`](../_shared/project-archive-sop.md) §二）；③ **分享前脱敏**——对外分享/外发前，`sessionKey` / `session id` 一律**截断为前 8 字符 + `…`**（或取哈希前 12 位），余额/可用性只留档位符号（✅/⚠️/❌）、**不留具体数值**；④ **保留期**——原文件仅在 `run/<项目名>/` 本地存在，项目清理即随之删除，交付物与归档包均不含原始遥测；⑤ **★ 默认脱敏（v2.12.41，回应外部扫描「primary status.md sanitized by default」）**——`sessionKey` / `session id` **写入时即截断**（前 8 字符 + `…`）**或取哈希前 12 位**，**不落原文**；余额/可用性一律只落档位符号（✅/⚠️/❌）。**不依赖事后脱敏**——「留原文 + 事后 redact」被扫描判为控制点在收集之后（收容不足）。（**v2.12.42 收紧**：`sessionKey`/`sessionId` 一律**不记录**，见 §4.7——截断规则仅兜底用于历史文件或意外出现的标识。）
> 真源 = [`../_shared/关键协议.md`](../_shared/关键协议.md) §遥测收容。
...
**架构**: 多 Agent 九角色流水线（固定，不设总开关）。**worker 接管记录**（每节点失败时填一行；无失败留空）：<角色 / 原因 timeout|failed|no_artifact / 接管者=主控 / L1 影响>。**不记录宿主配置明细、不记 deny 原文、不记录“加固状态”**——论衡不读取宿主配置，OpenClaw 平台负责多 Agent 运行与宿主工具策略。
...
> 自检口径见 [`permissions.md`](../permissions.md)「能力自检」与 [`dispatch-header.md`](../_shared/dispatch-header.md)「启动自检」：只核对本角色声明与当前会话工具面；实际调用未声明工具才阻断。**本表不记录宿主配置明细或“加固状态”**。
...
> **留档（快照副本 opt-in）**：项目结束时的快照副本写入 `run/<项目名>/audits/methodology-footprint-{项目名}.md` 属 **Phase 0 opt-in 能力**——主人未勾选 ⇒ **不创建快照副本**（不写盘），仅在交付说明中说明本字段未启用；已勾选时：只写新文件，不移动、不删除任何既有文件
...
> **v2.3.12 P0-3 → v2.12.42 重做**：原「扫本机模型 + 逐候选 1-token 探测 + 余额预检」**整体删除**（回应外部扫描：模型清单枚举与主动探测超出写作流水线必要范围）。改为**静态映射**：读一次 `session_status` 的**已配置模型清单**（只读元数据，非探测、不查余额），按 [`模型候选池.md`](../_shared/模型候选池.md) §二映射规则定档，写入下表后**不再探测**。
>
...
**运行时降级（v2.12.42，取代「静态存在性 + 动态可用性」双检查）**：「配置存在 ≠ 可用」的残余风险（教训 #236 原始场景）改由**运行时首败降级**承担：首次实际调用失败（401/403/429/配额）→ 子代理 `degraded` 自报 → 按候选池**下一档**重派（零产物决策树：同档 ≤2 次 → 降档）→ **配额耗尽类暂停等主人拍板**（行为预授权，fail-closed）。撞墙成本从「Phase 0 多次探测 spawn」降为「一次失败调用 + 自动降档」。
...
- **启动宽限期 = 2 分钟**（v2.12.27，回应实战跨主机暴露的 spawn 时序问题）：spawn 后 2 分钟内主控复验第 4 件不期望心跳；2 分钟后再检。不足宽限的「心跳缺失」= 误报，不作复验失分。
...
>
> **术语消歧（v2.12.18）**：与 §方法论足迹的**留档副本**区分——留档副本是工作流内**新增**一个快照文件（`methodology-footprint-*.md`），既不移动也不删除；结题归档是工作流外的项目整理动作。两者不混称同一个"归档"。
...
- **留档（opt-in）**：仅由 phase-order.yaml 的 `methodology_snapshot` 条件节点处理；Phase 0 未勾选则记录 `not_triggered`，不创建 `run/<项目名>/audits/methodology-footprint-{项目名}.md`（与 §5.8 工作流外的「结题归档」区分）
- **借鉴 deep-research-pro 的方法论透明**（论衡化）——论衡不是简单复制竞品，而是把方法论足迹当成论衡哲学的一部分：诚实透明 + 主人随时看清进度 + 借鉴但不依赖
- **实时面板默认启用；持久化快照需 opt-in**：方法论足迹面板实时更新；快照写入必须在 Phase 0 显式勾选。噪音控制走「按档位裁剪字段集」，不以默认创建快照替代同意门
