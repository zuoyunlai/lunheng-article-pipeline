...
### 4.6 模型健康度（实时，v2.3.13 能力抽象 + 自检）

> **v2.3.12 P0-3 模型自检**：Phase 0 启动时主控扫本机可用模型，每个能力档从候选池按优先级选第一个可用模型，写入下方「本轮可用模型」表

| 能力档 | 角色 | 候选池（按优先级） | **本轮实际** | 余额 |
|--------|------|-------------------|-------------|------|
| 检索 | T1 / T2 / T3 | deepseek-v4-flash → glm-4-flash → qwen3-coder | `<实际>` | ✅/⚠️/❌ |
| 分析写作 | T4 / T5 | deepseek-v4-pro → minimax-m3 | `<实际>` | ✅/⚠️/❌ |
| 批判审计 | T6 / T7 / G14 | claude-opus-5 → minimax-m3 → deepseek-v4-pro | `<实际>` | ✅/⚠️/❌ |
| 主控 | T0 | deepseek-v4-pro → deepseek-v4-flash | `<实际>` | ✅/⚠️/❌ |
