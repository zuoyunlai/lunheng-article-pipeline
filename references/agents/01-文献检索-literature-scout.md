...
- **T1b 定向回查子任务（教训 #129，+ #139；v2.13.0 P0-1 扩展双窗口）**：主控在 **Phase 1.5**（简报标注 [Dxx]/[Lxx] 待复核、或 t2_5 红数据未回溯）或 **Phase 4.3**（`t1b_targeted_review`：T5 修订说明或 T7 审计报告出现「待人工核验」标记，ECS 实战反馈新增）判定 `triggered` 后 spawn T1b；prompt 显式列「待回查编号（[Lxx]/[Dxx]/[Cxx]）+ 期望原始来源机构」，T1 必据 prompt 跑一轮定向 web_search。未触发时主控必须在 status.md 记录 `not_triggered`，不可静默省略该窗口：
  ...
  - **Phase 4.3 回查范围（v2.13.0 P0-1）**：T5 修订轮新增标「待人工核验」的 [Lxx]/[Dxx]/[Cxx] 引用（含 T7 审计报告列出的未核验条目）；回查仅验引用存在性与元数据（标题/作者/年份/DOI/URL），不重写正文；≤2 轮，耗尽走主人三选一（接受 + Acknowledged Limitations / 主人自行核验 / 删除未核验引用）。派发话术 = [`../dispatch/T1b-定向回查.md`](../dispatch/T1b-定向回查.md)。
...
- **中文数据源集成派发（教训 #174，主人实测反馈）**：主控 prompt 含以下指令（**默认关闭，Phase 0 勾选②学术元数据才加**；未勾选或选「④全部拒绝」时不加）。**完整 URL / 梯队说明见 [`../_shared/真源/中文数据源集成.md`](../_shared/真源/中文数据源集成.md)（单一真源，勿在此复制 URL，教训 #269）**：
  ...
  **实战触发条件**：**默认关闭，Phase 0 勾选②学术元数据后**主控 prompt 才加本段 —— **仅在「授权允许检索类外发」时生效**。
