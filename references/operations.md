...
| 能力档 | 角色 | 候选池（示例，本机已配） | 理由 |
|------|------|------|------|
| 检索 | 文献/数据/案例检索 | 便宜快（deepseek-v4-flash → glm-4-flash） | 机械检索，最便宜可靠 |
| 分析写作 | 分析/写作 | 强推理（deepseek-v4-pro → minimax-m3） | 推理与成文质量 |
| 批判审计 | 批判/审计 | 顶配（claude-opus-5 → minimax-m3 → deepseek-v4-pro） | 专抓错，必须顶配 |
| 主控 | 主控 | 稳定路由（deepseek-v4-pro → deepseek-v4-flash） | 判断与终检 |
