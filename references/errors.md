...
【怎么解决】1) 主控列出需替换的编号清单作为待处理项交主人确认  2) 修订稿必须先保留旧版再改（用 read+write 新建 v{N+1}，不覆盖原文件）  3) 写手铁律 #10 已要求逐项自检
...
| **友好版** | `❌ AI 使用声明缺失<br>【发生了什么】终稿文末没有「AI 使用声明」段<br>【为什么】所有论衡产出必须披露 AI 使用（教训 #273）<br>【怎么解决】1) 加「AI 使用声明」段：本文由论衡流水线生成 + 检索真实可追溯  2) 不是免责声明，是诚实透明` |
...
| **友好版** | `⚠️ 早期框架锁定警告<br>【发生了什么】大纲 5 个论点全部服务同一框架，反方论证不够<br>【为什么】F3 防御（教训 #270）<br>【怎么解决】1) 分析员补 1-2 个「反方论证」段落  2) 拆分主线为 2 个子框架  3) 不影响交付，但写手 Phase 3 需更平衡` |
...
| **友好版** | `❌ Phase 0 同意关卡缺失<br>【发生了什么】主控在没确认主人同意的情况下直接 spawn T1<br>【为什么】强制金标准（教训 #266）<br>【怎么解决】1) 主控立即给主人 4 选 1  2) 等主人选择后再继续  3) 这是 user-facing 警告，不应静默跳过` |
...
| **友好版** | `📋 Acknowledged Limitations 模式触发<br>【发生了什么】修订第 3 轮仍有 P0/P1 未关闭<br>【为什么】硬约束（教训 #270）—— 防止无限循环<br>【怎么解决】1) 主控 T8 把未关闭项搬入 final/局限性.md  2) 文末列「未关闭 P0/P1」清单  3) 论文正常交付（不假装完美）` |

#### E13：产物未落盘 / 零产物（v2.12.40 新增）

| 项 | 内容 |
|---|---|
| **触发** | 子代理声称完成，但其产物文件**不存在** / **字节数为 0**；或闸门读到的文件 mtime 早于上游完成时间（旧文件冒充新产物） |
| **原报错** | `artifact missing` / `zero-byte artifact` / `stale input` |
| **友好版** | `📦 产物未落盘（零产物）<br>【发生了什么】<角色> 报告已完成，但 <路径> 不存在 / 为空 / 是上一轮的旧文件<br>【为什么】静默失败（与教训 #274 同型）：子代理 ack 完成但未真正 write；或主控内存拼接后未落盘；或批改只更新了 run/ 未更新 outputs/<br>【怎么解决】按**闸门第 0 步**（见 M-Gate-Algorithm.md）判「**路径或参数错误**」档 —— **不触发内容修订轮** → ①同档重派（上限 2 次）②降档重派 ③仍失败 → 暂停问主人。**禁止**把「产物不存在」当内容缺陷（白烧一轮修订）` |

> 与 E12 的区别：E12 是「内容真的改不好」（该走 Acknowledged Limitations）；E13 是「产物根本没到位」（该修路径/参数重跑）—— **两者处置完全不同，不得混用**。
...
【发生了什么】D05 没有标注「🟢/🟡/🔴」数据来源
【为什么】v2.2.1 起所有数据必须标信任级别（防 F8.4 教训 #271）
