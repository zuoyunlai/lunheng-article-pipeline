...
| **友好版** | `📦 产物未落盘（零产物）<br>【发生了什么】<角色> 报告已完成，但 <路径> 不存在 / 为空 / 是上一轮的旧文件<br>【为什么】静默失败（与教训 #274 同型）：子代理 ack 完成但未真正 write；或主控内存拼接后未落盘；或批改只更新了 run/ 未更新 outputs/<br>【怎么解决】按**闸门第 0 步**（见 M-Gate-Algorithm.md）判「**路径或参数错误**」档 —— **不触发内容修订轮** → ①同档重派（上限 2 次）②降档重派 ③仍失败 → 暂停问主人（**本树仅限「未启动 / spawn 丢失」（tokens=0 ∧ history 停在初始 prompt）；已启动但硬卡静默的零产物 ⇒ 主控兜底、不重试**，见 [`agents/00-主控-扩展职责.md`](agents/00-主控-扩展职责.md)「§不通过处理」）。**禁止**把「产物不存在」当内容缺陷（白烧一轮修订）` |
