...
if [ "$PURGE_GIT_HISTORY" != "true" ]; then
  ...
  UNREACHABLE_BEFORE=$(git fsck --no-reflogs --unreachable --no-progress 2>&1 | grep -cE '^(unreachable|dangling|不可达|悬空)' || true)
  ...
  UNREACHABLE_AFTER=$(git fsck --no-reflogs --unreachable --no-progress 2>&1 | grep -cE '^(unreachable|dangling|不可达|悬空)' || true)
