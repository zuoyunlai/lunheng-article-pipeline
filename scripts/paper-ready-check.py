...
本脚本 = references/_shared/真源/可发表性判定表.md 的本地开发者版执行器。
...
# ============================================================
# 判据函数（与 references/_shared/真源/可发表性判定表.md § 一伪代码对应）
# ============================================================
...
def check_residual_codes(final_md_path: str) -> tuple:
    ...
    patterns = [
        ...
        # v2.12.42：与判定表 A2 扩充对齐——交付版实测漏检的六类
        r'(?:\[|（)A[1-9](?:\]|）)',        # 断言编号（[A4] / （A2））
        r'T\d+\s*分析员|T6 批判报告|G14 检测',  # 角色 / 角色卡名
        r'降级纪律|降级模式|信任级别|主人投喂',   # 流程术语
        r'≥\s*\d+\s*字',                  # 流程配额
        r'\d{1,2}/30|Minor Revision|维度评分',  # 自评分与内评结论
        r'感谢\s*[0-9一二三四五六七八九十]+\s*位匿名审稿人',  # 虚假致谢
        r'\[(?:L|D|B|C)\d+\]',             # 内部卡/证据编号（仅定稿内部版允许，投稿版必须为 0）
    ]
    hits = []
    for pat in patterns:
