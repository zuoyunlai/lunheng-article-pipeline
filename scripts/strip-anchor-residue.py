...
# 注意 1：跳过正则字符组 `[^（）]`（M-Gate-Algorithm 的内联引用匹配式依赖它）
# 注意 2（教训 #300）：空括号规则必须排除「函数调用」形态 `标识符()`，
#   否则会删掉代码块内所有无参函数调用的括号：
#   `Path(base_dir).resolve()` → `.resolve`、`c.strip()` → `c.strip`、
#   `time.now()` → `time.now`。本规则本意只清「剥离后残留的空全角括号」。
#   保护方式：`(` 紧跟 ASCII 标识符 / `.` 时视为调用，不动。
#   实测损伤面：v2.12.4 / v2.12.5 净化包 7 文件 13 处（含 ClawHub 审计 v2.12.5 Finding #1 所指的
#   路径校验伪代码），句式与代码语义静默损坏，扫描器不报错。
JUNK_TIDY = [
    # 全角空括号：剥离锚点后的残留（`（教训 #N）` → `（）`），无条件删
    (re.compile(r'(?<!\^)（[ \t　]*）(?!\])'), ''),
    # 半角空括号：必须排除函数调用形态 `标识符()`（教训 #300），
    # 否则代码块内 `resolve()` / `c.strip()` / `time.now()` 的括号被吃→ 代码语义静默损坏
