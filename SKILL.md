...
词汇表集中定义了：
- 9 个角色职责（T0-T8，v2.3.0 重构：原 8 角色 → 9 角色，原 T6 案例检索 → T3，T6 = 批判伙伴，T7 = 审计员，T8 = 终检）
...
| 角色 | 推荐模型 | 理由 |
|------|---------|------|
| T1 / T2 / T3 检索类 | deepseek-v4-flash / subagent_retrieval | 检索任务是抽取+分类，便宜快足够（v2.3.0 改 T6→T3） |
| T4 分析 + T5 写手 | deepseek-v4-pro / minimax-M3 | 分析写作需强推理（v2.3.0 改 T3→T4/T4→T5） |
| T7 审计 + T6 批判伙伴 | minimax-M3 / Claude-Opus | 审计顶配防漏判（v2.3.0 改 T5→T7/T8→T6） |
...
2. **人在环四节点**：Phase 0（定题）、Phase 2.5（大纲）、Phase 3.5（洞察补充）、Phase 5（终稿）必须让主人过目
   - **v2.3.0 补充**：Phase 3.5 → Phase 3.6（T6 批判）→ Phase 4（T7 审计）→ Phase 5（T8 终检，主控亲完成）是连贯流水线，Phase 3.6 也算「人在环」隐式触发（v2.3.0 重构明确）
