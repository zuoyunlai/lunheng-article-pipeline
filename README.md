...
**v2.12.32**（2026-09-12，当前版本；以 [`CHANGELOG.md`](CHANGELOG.md) 首节为准）**v2.12.32 无人值守实测修订**（**P0×2** + **P1×3** + **P2×2**）：**工具面/调用两级判据拆分**（工具面超限＝警告、实际调用＝阻断，解 `denied` 声明「自锁陷阱」）+ **Phase 0 字数预算机械自查**（解「上限与分章硬指标互斥」永不关闭 P0）+ **G14 复检收敛判据 + Phase 0 口径固定** + **M-Form-7 白名单补「致谢」**（解与可发表性 F 选项互斥）+ 机械搬运合并 + T5 读卡纪律/900s 硬卡；叠加官方规范比对修复（`denied` 19 → 27 项 + 记忆外发口径 + `cwd` 口径统一），SKILL.md 体量棘轮再下调 11384 → **11370**。此前 v2.12.31 = ClawHub 扫描复核修复（AE1 HIGH + SQP-2 / SQP-1 两项 MEDIUM）+ 门 U 扩第二类检查（入口文档裸文件引用）+ 抓取层第三方外发警示。沿革见 CHANGELOG 对应章节。[MIT License](LICENSE)。
...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.32  # pin 审计版本（回应 ClawHub T08）
