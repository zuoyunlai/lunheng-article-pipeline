...
**v2.12.67**（2026-09-21，当前版本；以 [`CHANGELOG.md`](CHANGELOG.md) 首节为准）——**审计第七批整改（P1×3 + P2×5）**：只读档超长报告**分片预申报协议**（flow-check 规则 42）+ 性能基准四维登记表 + flow-check 规则集**元健康度门**；P2 五项：平台升级冒烟 / glossary 漂移清理 / 心跳归属标识 / README 版本块容量门 / G14 时点有效性诚实边界。详见 CHANGELOG §v2.12.67。
...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.67  # pin 审计版本（回应 ClawHub T08）
