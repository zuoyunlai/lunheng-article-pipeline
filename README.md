...
- ❌ 不调用 `exec` / `process` / `code_execution` 等**39 项 denied 特权工具**（清单真源 = `SKILL.md` frontmatter `metadata.tools.denied`）——⚠️ **声明式**，须宿主配置才生效
...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.38  # pin 审计版本（回应 ClawHub T08）
