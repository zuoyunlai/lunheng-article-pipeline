...
**v2.12.11**（2026-09-10，当前版本）— 全仓四路专项审计（口径漂移 / 执行衔接 / 净化链内泄漏 / 交叉引用）真问题全修：加固口径统一为 fail-closed、人在环超时兜底、status.md 写入者收口、G14/T7/T9 时序校正、净化链残留规则补漏。[MIT License](LICENSE)。
...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.25  # pin 审计版本（回应 ClawHub T08）
