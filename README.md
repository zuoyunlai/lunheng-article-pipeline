...
> 🔐 **权限设计取舍（对审计方/扫描器明示）**：论衡是**纯 skill**，不附带宿主配置项（B 档）。子代理角色白名单是**声明与部署建议**，真实强制由宿主工具策略承担（加固配方见 `references/_shared/host-hardening-recipe.md`）。未加固宿主上「工具面超限」属**预期常态**：论衡的处置 = 记录 + 披露 + **主人裁决**（敏感题材自动降单主控）+ 铁律「**工具面超限 ≠ 调用许可**」；「无加固即拒跑」已被实测否决（v2.12.32 并行层自锁零产物）。**这是有意的设计取舍，不是未修复的缺陷。**
...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.12.44  # pin 审计版本（回应 ClawHub T08）
