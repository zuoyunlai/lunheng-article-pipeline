...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.7.14  # pin 审计版本（回应 ClawHub T08，v2.7.14）
...
| **v2.7.14** | **2026-09-07** | **当前** | **ClawHub v2.7.13 审计 2 findings 回应（Outcome Review）：T05 运行前软保障自检（Phase 0 主控自查工具面 + 三态确认 + status.md 软保障标记，不拒绝运行）+ T08 安装命令 pin 审计版本；ClawHub 暂缓上架** |
