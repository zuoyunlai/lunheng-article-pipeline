...
**v2.12.29**（2026-09-12，当前版本；以 [`CHANGELOG.md`](CHANGELOG.md) 首节为准）三版连修：v2.12.27 真问题全修（fail-closed 加固 / 人在环超时兜底 / status.md 写入者收口 / G14-T7-T9 时序校正）→ v2.12.28 实测诊断全修 + 文档全量审计 + 流程审计（配图节点可达性 + **新增门 S**）→ v2.12.29 审计页回归修复（T09 同意绕过 / 机器语义 / 声明不符）。[MIT License](LICENSE)。
