...
**v2.12.10**（2026-09-10，当前版本）— ClawHub 对已发布 v2.12.9 净化包的语义审计（15 条）5 类真问题全修 + 加固模型收紧为二选一（mechanical / degraded）+ 净化链代码保真修复 + Release 单一入口脚本入库。[MIT License](LICENSE)。
...
| **v2.12.10** | **2026-09-10** | **当前** | **ClawHub 语义审计（对象＝已发布 v2.12.9，15 条）5 真问题全修 + 加固模型收紧为二选一 + 净化链代码保真（教训 #300）+ Release 单一入口脚本** |
