...
Phase 4.2 修订回环   写手修订稿常规 ≤2 轮（v2.2.0 硬约束，例外通道须主人拍板）
...
**方式一（推荐）**：ClawHub 安装
```bash
openclaw skills install @zuoyunlai/lunheng-article-pipeline@2.7.15  # pin 审计版本（回应 ClawHub T08，v2.7.14）
...
| v2.7.2 | 2026-09-07 | 审计修复 | 全面独立审计修复：只读档报告回传机制（P0）/ G14 时序统一到 phase-order.yaml（P0）/ 死链 + 表述漂移清理（P1/P2） |
| v2.7.3 | 2026-09-07 | 实战复盘 | ECS v2.6.9 复盘 20 条改进落地（P0×4：静默降级检测 / 修订轮仲裁表 / 字数闭合 / 导出元数据清洗 + P1×8 + P2×8） |
...
| **v2.7.15** | **2026-09-08** | **当前** | **v2.7.4 实测报告 4 项真实缺口落地（本机逐项核对保留项）：R4 报告长度硬上限（phase-order.yaml `output_chars_max`：T4/T6 ≤12k、T7 ≤10k、G14 ≤6k、T9 ≤15k、T8 ≤10k）+ 超限分块协议 + fallback A/B/C 质量分级（韧化协议 4.7）；R7 反方回应 ≥200 字硬规定 + 4 要素模板（字数判定表 § 〇，T7 P1 打回）；R2 元叙事自指式豁免段（结构必写段豁免：先行者差异化/AI 声明/摘要限定语/主人洞察理论贡献，写手卡/T6/T7/G14 checker+gate 同源）；R9 警示 ⚠️→▲/【警告】 + PDF 渲染验证（format-export §四·六）；ClawHub 暂缓上架** |
| v2.7.14 | 2026-09-07 | 审计回应 | ClawHub v2.7.13 审计 2 findings 回应（Outcome Review）：T05 运行前软保障自检（Phase 0 主控自查工具面 + 三态确认 + status.md 软保障标记，不拒绝运行）+ T08 安装命令 pin 审计版本；ClawHub 暂缓上架 |
| v2.7.13 | 2026-09-07 | 权限软化 | 权限表述软化为建议口径（接教训 #202）：宿主 config 收紧从「必须」降为「建议」——不收紧时论衡照常运行，零 exec 为纪律层软保障（全文档零授权 + M 门扫描 + 外部内容不可信）非机械强制，与「纯 skill 任意配置可用」定位一致；SKILL/QUICKSTART/dispatch 三处同步 |
