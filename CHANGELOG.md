...
- **配套 4 条单测 `tests/test_gate_x_fence_phase.py`**：① 基线正向验证（修复后仓库 X.1/X.2/X.3 全 ✓）；② X.2 变异验证（在 M-Gate 末尾追加被围栏裹的 M-Form-99 标题，门 X.2 必须 ✗）；③ X.1 变异验证（删一个裸围栏让总数变奇数，门 X.1 必须 ✗）；④ X.3 变异验证（追加「紧跟围栏关闭的 # 伪 H1」，门 X.3 必须 ✗）—— **变异必须被抓到才算门真有效**（避免 #175/#421 同型「门只描述不执行 → 静默空转」）。
- **验收（实测回填 · 最终态）**：自审门 **PASS 29 / FAIL 0**（门 X.1/X.2/X.3 全绿 + 既有 26 门无回退）；`python3 -m pytest tests/ -q` → **373 passed**（本批新增 4 条）；`bash -n scripts/self-audit-gate.sh` 通过（门 X 段 awk 把 `\x60` 字面化后无 POSIX awk 兼容性问题）；围栏修复后 `references/_shared/M-Gate-Algorithm.md` 行数 1250 → 1248（删 2 行）；同步版本戳 **89 文件**。
- **教训库同批同步（#424-#426 三条入库 + 编号修正）**：主真源续录 **#424**（裸 grep/awk 不认围栏，标题编号从错写的 #423 修正）/ **#425**（trigger 脚本 exec 大 JSON 静默不报）/ **#426**（围栏错位 → 偶数必要不充分 → 门 X 三道）；`references/_shared/lessons-max.snapshot` 423 → **425**（载明 423→425 的更新记录）；`references/_shared/教训索引.md` 三处副本同批（§一 / §二 / §三 完整教训库 + 新增 §三点八批次 G 分类补记）；排除表 `LUNHENG_LESSON_EXCLUDE` **未变** —— 三条均属论衡类（F 审计/分析方法 + 工程实践）。
