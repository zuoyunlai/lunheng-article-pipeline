...
- **方法论透明度深化**：新增 G18 方法论审计真源、§3.5 研究方法设计、§4 研究方法章节模板与落地 walkthrough；T7 增加 G18 必查，T9 增加 D2 方法论评分。
- **理论原创性深化**：新增理论贡献协议与三层声明/L1-L4 层级判定；T4 增加 §2.3 理论贡献声明，T6 增加 C1 L1-L4 复核，T9 增加 D1 理论贡献评分。
- **G14 定位升级**：新增段落级密度热力图、Top 5 高密度段与全文加权密度；保留原 9 类总判定作为最终 Pass/Warning/Fail 依据。
- **T9b 压力测试轮**：新增主控亲完成的三剧本压力测试（方法论、立场与范围、跨语境），默认触发且可 opt-out，不替代 T6/T7/T9。
- **流程真源接线**：节点由 24 扩展为 25，`t9b_stress_test` 插入 T9 与 T8 之间；版本号统一为 2.14.0。
